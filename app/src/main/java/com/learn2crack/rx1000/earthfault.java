...
public class earthfault extends BaseActivity implements TagDiscovery.onTagDiscoveryCompletedListener{
    ...
    private static final String TAG = earthfault.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLayoutInflater().inflate(R.layout.earthfault, contentFrameLayout);
        earthfault.this.setTitle("EARTH-FAULT SETTING");
        mNfcAdapter = NfcAdapter.getDefaultAdapter(this); //check NFC hardware available
        mPendingIntent = PendingIntent.getActivity(this, 0, new Intent(this, getClass()).addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP), 0);

        readbtn = findViewById(R.id.btn_read_ef);
                ...
                Log.d(TAG, "Mikro sent: " + request);
                    ...
                    if (spin2.equals("OFF")){
                        ...
                        if (TextUtils.isEmpty(editIo2.getText().toString())) {
                            ...
                            get_Io2 = StepParam.getstep2(get_Io2_d, 10, 5000);
                            Log.d(TAG, "Mikro result Io2 setting from apps after process " + get_Io2);
                            if (get_Io2 == 0) {
                                if (get_Io2_d < 0.1) {
                                    editIo2.setError("Io>> setting cannot be less than 0.1");
                                } else if (get_Io2_d > 50) {
                        ...
                        if(((ST25DVTag)mftmTag).isMailboxEnabled(false))
                        {
                            ...
                            runOnUiThread(new Runnable() {
                                public void run() {
                                    writebtn.setBackgroundColor(getResources().getColor(R.color.colorAccent));
                                    //writebtn.setBackground(getResources().getDrawable(R.drawable.rouded_button));
                                    Toast.makeText(earthfault.this, "FTM OFF", Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                                    ...
                                    if (!result){
                                        runOnUiThread(new Runnable() {
                                            public void run() {
                                                ...
                                                new AlertDialog.Builder(earthfault.this)
    ...
    private int sendSimpleData() {
        ...
        try {
            ...
            if (((ST25DVTag)mftmTag).hasRFPutMsg(true)) {
                ...
                runOnUiThread(new Runnable() {
                    public void run() {
                        Toast.makeText(earthfault.this, "RF Mailbox Not Empty", Toast.LENGTH_SHORT).show();
                    }
                });
                reset_ftm();
